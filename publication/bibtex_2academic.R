...
bibtex_2academic <- function(bibfile,
  ...
  min_row= mypubs$index %>% min
  max_row= mypubs$index %>% max
  # browser()
  ...
  create_md <- function(x) {
    ...
    if (!file.exists(file.path(outfold, filename)) |
      ...
      # browser()
      # if(as.numeric(x[['index']])== min_row){
      #   write('---', fileConn)
      # }else write('---', fileConn, append = T)
        # write(paste0('- "',as.numeric(x[['index']]), '"'), fileConn,append = T)
      write("+++", fileConn)
...
my_bibfile <- "content/publication/journal-article/cite.bib"
